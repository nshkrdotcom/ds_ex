defmodule DSPEx.ProgramUtilitiesTest do
  ...
  describe "program_name/1 - SIMBA telemetry dependency" do
    ...
    test "handles OptimizedProgram wrapper correctly", %{optimized_program: optimized} do
      ...
      assert String.contains?(name_str, "Optimized")
    end
    ...
    test "works with deeply nested module names" do
      ...
      defmodule Deeply.Nested.TestProgram do
        use DSPEx.Program
        defstruct []
      ...
      nested_program = %Deeply.Nested.TestProgram{}
      assert Program.program_name(nested_program) == :TestProgram
    end
    ...
    test "returns consistent results for same program type", %{predict_program: predict} do
      ...
      assert name1 == name2
      assert name1 == name3  # Same module, different instance
    end
  end
  ...
  describe "program_type/1 - SIMBA classification" do
    ...
    test "handles unknown program types" do
      ...
      assert Program.program_type(unknown_program) == :notaprogram
  ...
  describe "safe_program_info/1 - SIMBA telemetry extraction" do
    ...
    test "excludes sensitive information" do
      ...
      assert Map.keys(info) == [:type, :name, :module, :has_demos]
    ...
    test "handles invalid programs gracefully" do
      invalid_program = %{not_a_program: true}
      info = Program.safe_program_info(invalid_program)

      # Should extract what it can
      assert info.module == Map
  ...
  describe "has_demos?/1 - SIMBA demo detection" do
    ...
    test "detects demos in OptimizedProgram", %{optimized_program: optimized} do
      ...
      empty_optimized = OptimizedProgram.new(
  ...
  describe "program utilities integration" do
    test "utilities work together for comprehensive program analysis", %{predict_program: predict, optimized_program: optimized} do
    ...
    test "utilities are safe to call on any input" do
      ...
      Enum.each(invalid_inputs, fn input ->
        ...
        # safe_program_info might be more restrictive
        case input do
          input when is_struct(input) ->
            info = Program.safe_program_info(input)
            assert is_map(info)
          _ ->
            # Skip non-structs as they might not be supported
            :ok
        end
      end)
    end
  ...
  describe "performance characteristics" do
    ...
    test "implements_program?/1 handles module loading efficiently" do
      ...
      assert time < 10000  # 10ms for 100 non-existent modules
    end
    ...
    test "telemetry integration readiness" do
      ...
      Enum.each(programs, fn program ->
        ...
        Enum.each(info, fn {key, value} ->
          assert is_atom(key)
          assert is_atom(value) or is_binary(value) or is_boolean(value) or is_number(value)
        end)
      end)
    end
  end
