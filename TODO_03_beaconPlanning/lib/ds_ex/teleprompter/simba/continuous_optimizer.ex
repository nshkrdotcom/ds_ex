defmodule DSPEx.Teleprompter.SIMBA.ContinuousOptimizer do
  ...
  def handle_info(:quality_check, state) do
    ...
    if needs_optimization do
      IO.puts("⚡ Quality check triggered immediate optimization (quality: #{Float.round(current_quality, 3)})")
      send(self(), :optimize)
    else
  ...
  def handle_info(:optimize, state) do
    ...
    case run_optimization(state) do
      {:ok, optimized_program, improvement_metrics} ->
        ...
        IO.puts("✅ Optimization completed (improvement: +#{Float.round(improvement_metrics.improvement_percentage, 1)}%)")
      ...
      {:error, reason} ->
        ...
        retry_delay = min(state.optimization_interval, 30 * 60 * 1000)  # Max 30 minutes
  ...
  def terminate(reason, state) do
    IO.puts("🔚 Continuous optimizer stopped (ID: #{state.correlation_id}, reason: #{inspect(reason)})")
    :ok
  end
  ...
  defp should_optimize_immediately?(current_quality, state) do
    ...
    # Check quality trend
    quality_declining = is_quality_declining?(state.quality_history)

    # Check time since last optimization
    hours_since_optimization = DateTime.diff(DateTime.utc_now(), state.last_optimization, :hour)
    overdue = hours_since_optimization >= 48  # More than 2 days

    quality_below_threshold or (quality_declining and hours_since_optimization >= 12) or overdue
  end

  defp is_quality_declining?(quality_history) when length(quality_history) < 3 do
    false
  end

  defp is_quality_declining?(quality_history) do
