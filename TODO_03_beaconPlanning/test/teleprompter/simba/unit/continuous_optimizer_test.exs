defmodule DSPEx.Teleprompter.SIMBA.ContinuousOptimizerTest do
  ...
  describe "GenServer initialization" do
    test "starts with proper initial state", %{program: program} do
      {:ok, pid} = ContinuousOptimizer.start_link([
        program: program,
    ...
    test "accepts all configuration options", %{program: program, trainset: trainset, teacher: teacher, metric_fn: metric_fn} do
      opts = [
        program: program,
  ...
  describe "client API" do
    setup %{program: program} do
      {:ok, pid} = ContinuousOptimizer.start_link([
        program: program,
        optimization_interval: 5000,  # Longer intervals for testing
  ...
  describe "quality monitoring" do
    test "quality check with sufficient configuration", %{program: program, trainset: trainset, teacher: teacher, metric_fn: metric_fn} do
      {:ok, pid} = ContinuousOptimizer.start_link([
        program: program,
        trainset: trainset,
        teacher: teacher,
        metric_fn: metric_fn,
        quality_check_interval: 50,  # Very short for testing
    ...
    test "quality trend analysis" do
      ...
      assert trend == :improving  # Quality is increasing over time
    ...
    test "immediate optimization trigger conditions" do
      ...
      assert should_optimize_immediately_logic(0.5, %{quality_threshold: 0.7, last_optimization: DateTime.utc_now()})

      # Quality declining trend with recent check
      recent_time = DateTime.add(DateTime.utc_now(), -10, :hour)
      declining_state = %{
        quality_threshold: 0.7,
        last_optimization: recent_time,
        quality_history: [
          %{quality: 0.6}, %{quality: 0.7}, %{quality: 0.8}  # Declining
        ]
      }
  ...
  describe "optimization execution" do
    test "optimization with proper configuration", %{program: program, trainset: trainset, teacher: teacher, metric_fn: metric_fn} do
      ...
      {:ok, pid} = ContinuousOptimizer.start_link([
        ...
        teacher: teacher,
        metric_fn: metric_fn,
        optimization_interval: 50,  # Short interval for testing
        quality_check_interval: 2000
      ])
    ...
    test "optimization intensity adjustment based on quality" do
      ...
      assert low_intensity == 30  # 20 * 1.5
      assert high_intensity == 16  # 20 * 0.8
    end
  ...
  describe "start_optimization convenience function" do
    test "starts optimization with default intervals", %{program: program} do
      ...
      state = :sys.get_state(pid)
      assert state.program == program
      assert state.optimization_interval == 24 * 3600 * 1000  # 24 hours
      assert state.quality_check_interval == 6 * 3600 * 1000   # 6 hours
    ...
    test "accepts custom intervals", %{program: program} do
      ...
      state = :sys.get_state(pid)
      assert state.optimization_interval == 12 * 3600 * 1000  # 12 hours
      assert state.quality_check_interval == 3 * 3600 * 1000   # 3 hours
  ...
  defp analyze_quality_trend_logic(quality_history) do
    recent_qualities = quality_history
    |> Enum.take(5)
    |> Enum.map(& &1.quality)
    |> Enum.reverse()
  ...
  defp determine_trial_count_logic(quality_history) do
    ...
    recent_failures = quality_history
    |> Enum.take(5)
