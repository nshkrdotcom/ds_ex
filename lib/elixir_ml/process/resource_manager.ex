defmodule ElixirML.Process.ResourceManager do
  ...
  @default_cleanup_interval_ms 60_000  # 1 minute
  ...
  def init(opts) do
    ...
    schedule_monitoring(5000)  # Monitor every 5 seconds
  ...
  def handle_cast({:release_resources, _pid}, state) do
    ...
    new_state = %{state |
      current_process_count: max(0, state.current_process_count - 1)
    }
  ...
  defp perform_cleanup(state) do
    ...
    Logger.info("Resource cleanup completed. Cleared #{length(state.alerts) - length(new_alerts)} old alerts")
  ...
  defp check_resource_alerts(state, current_memory_mb) do
    ...
    # Memory usage alerts
    memory_threshold = state.max_memory_mb * 0.8  # 80% threshold
    alerts = if current_memory_mb > memory_threshold do
      alert = %{
        type: :high_memory_usage,
        severity: :warning,
        message: "Memory usage is #{Float.round(current_memory_mb, 2)}MB (#{Float.round(current_memory_mb / state.max_memory_mb * 100, 1)}%)",
        timestamp: System.monotonic_time(:millisecond)
      }
      [alert | alerts]
    else
    ...
    process_threshold = state.max_processes * 0.8  # 80% threshold
    alerts = if state.current_process_count > process_threshold do
      alert = %{
        type: :high_process_count,
        severity: :warning,
        message: "Process count is #{state.current_process_count} (#{Float.round(state.current_process_count / state.max_processes * 100, 1)}%)",
        timestamp: System.monotonic_time(:millisecond)
      }
      [alert | alerts]
    else
