defmodule DSPEx.Config.ElixirMLSchemasTest do
  ...
  describe "path_to_schema/1" do
    test "returns client configuration schema for client paths" do
      assert {:ok, schema, [:timeout]} = ElixirMLSchemas.path_to_schema([:dspex, :client, :timeout])
    ...
    test "handles nested configuration paths" do
      assert {:ok, schema, [:requests_per_minute]} =
               ElixirMLSchemas.path_to_schema([:dspex, :providers, :openai, :rate_limit, :requests_per_minute])
      assert Map.has_key?(schema, :fields)
    end
  end
  ...
  describe "validate_config_value/2" do
    ...
    test "rejects invalid client timeout" do
      assert {:error, _} = ElixirMLSchemas.validate_config_value([:dspex, :client, :timeout], "invalid")
      assert {:error, _} = ElixirMLSchemas.validate_config_value([:dspex, :client, :timeout], -1000)
      assert {:error, _} = ElixirMLSchemas.validate_config_value([:dspex, :client, :timeout], 0)
    end

    test "validates provider configuration" do
      assert :ok = ElixirMLSchemas.validate_config_value([:dspex, :providers, :openai, :api_key], "sk-test")
      assert :ok = ElixirMLSchemas.validate_config_value([:dspex, :providers, :openai, :api_key], {:system, "OPENAI_API_KEY"})
      assert :ok = ElixirMLSchemas.validate_config_value([:dspex, :providers, :openai, :base_url], "https://api.openai.com/v1")
    end

    test "validates prediction configuration with ML types" do
      # Temperature should use probability-like validation
      assert :ok = ElixirMLSchemas.validate_config_value([:dspex, :prediction, :default_temperature], 0.7)
      assert {:error, _} = ElixirMLSchemas.validate_config_value([:dspex, :prediction, :default_temperature], 3.0)

      # Provider should be atom choice
      assert :ok = ElixirMLSchemas.validate_config_value([:dspex, :prediction, :default_provider], :openai)
      assert {:error, _} = ElixirMLSchemas.validate_config_value([:dspex, :prediction, :default_provider], :invalid)
    end

    test "validates boolean configurations" do
      assert :ok = ElixirMLSchemas.validate_config_value([:dspex, :prediction, :cache_enabled], true)
      assert :ok = ElixirMLSchemas.validate_config_value([:dspex, :prediction, :cache_enabled], false)
      assert {:error, _} = ElixirMLSchemas.validate_config_value([:dspex, :prediction, :cache_enabled], "true")
    end
    ...
    test "validates logging configuration" do
      ...
      assert {:error, _} = ElixirMLSchemas.validate_config_value([:dspex, :logging, :level], :invalid)
    end

    test "validates BEACON teleprompter configuration" do
      assert :ok = ElixirMLSchemas.validate_config_value([:dspex, :teleprompters, :beacon, :max_concurrent_operations], 10)
      assert {:error, _} = ElixirMLSchemas.validate_config_value([:dspex, :teleprompters, :beacon, :max_concurrent_operations], 0)

      # Bayesian optimization parameters
      assert :ok = ElixirMLSchemas.validate_config_value([:dspex, :teleprompters, :beacon, :bayesian_optimization, :acquisition_function], :expected_improvement)
      assert {:error, _} = ElixirMLSchemas.validate_config_value([:dspex, :teleprompters, :beacon, :bayesian_optimization, :acquisition_function], :invalid)
    end
    ...
    test "returns error for unknown paths" do
      assert {:error, {:unknown_path, _}} = ElixirMLSchemas.validate_config_value([:invalid, :path], "value")
    end
  end
  ...
  describe "schema validation with ElixirML types" do
    test "client configuration schema uses proper ElixirML types" do
      ...
      assert {:ok, validated} = ElixirML.Runtime.validate(schema, %{
        timeout: 30_000,
        retry_attempts: 3,
        backoff_factor: 2.0
      })
    ...
    test "prediction configuration uses ML-native types" do
      {:ok, schema, _} = ElixirMLSchemas.path_to_schema([:dspex, :prediction, :default_temperature])
  ...
  describe "error handling and compatibility" do
    test "maintains error format compatibility with SinterSchemas" do
      {:error, {error_atom, message}} = ElixirMLSchemas.validate_config_value([:dspex, :client, :timeout], "invalid")
    ...
    test "provides detailed validation errors" do
      {:error, {error_atom, message}} = ElixirMLSchemas.validate_config_value([:dspex, :prediction, :default_temperature], 5.0)
