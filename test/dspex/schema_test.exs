defmodule DSPEx.SchemaTest do
  ...
  defmodule ConstrainedSignature do
    use DSPEx.Signature, "name:string[min_length=2,max_length=50] -> greeting:string[max_length=100]"
  end

  defmodule ComplexSignature do
    use DSPEx.Signature, "context:string[min_length=10], question:string[min_length=5] -> answer:string, confidence:float, reasoning:string[max_length=500]"
  end
  ...
  describe "signature_to_schema/1" do
    test "converts basic DSPEx signature to ElixirML schema" do
      ...
      assert answer_field.required == false  # outputs are not required for validation
    end
  ...
  describe "validate_with_elixir_ml/3" do
    test "validates input data against signature" do
      ...
      {:ok, validated} = DSPEx.Schema.validate_with_elixir_ml(TestSignature, input_data, field_type: :inputs)
    ...
    test "validates output data against signature" do
      ...
      {:ok, validated} = DSPEx.Schema.validate_with_elixir_ml(TestSignature, output_data, field_type: :outputs)
      
      assert validated.answer == "2+2 equals 4"
    end

    test "validates constraint violations" do
      invalid_data = %{name: "A"}  # Too short (min_length=2)
      
      {:error, errors} = DSPEx.Schema.validate_with_elixir_ml(ConstrainedSignature, invalid_data, field_type: :inputs)
      
      assert length(errors) > 0
      assert Enum.any?(errors, fn error -> String.contains?(error.message, "too short") or String.contains?(error.message, "minimum") end)
    end
    ...
    test "validates ML-specific types" do
      ...
      {:ok, validated} = DSPEx.Schema.validate_with_elixir_ml(ComplexSignature, valid_data, field_type: :inputs)
    ...
    test "validates float fields" do
      ...
      {:ok, validated} = DSPEx.Schema.validate_with_elixir_ml(ComplexSignature, output_data, field_type: :outputs)
    ...
    test "handles missing required fields" do
      ...
      {:error, errors} = DSPEx.Schema.validate_with_elixir_ml(TestSignature, empty_data, field_type: :inputs)
  ...
  describe "backward compatibility" do
    ...
    test "error format compatible with existing error handling" do
      invalid_data = %{name: "A"}  # Too short - this should trigger a validation error
      
      result = DSPEx.Schema.validate_with_elixir_ml(ConstrainedSignature, invalid_data, field_type: :inputs)
