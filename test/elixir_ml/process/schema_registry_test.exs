defmodule ElixirML.Process.SchemaRegistryTest do
  ...
  describe "caching functionality" do
    ...
    test "can cache and retrieve validation results" do
      ...
      # Cache the result
      SchemaRegistry.cache_validation_result(schema_module, data_hash, validation_result, :test_schema_cache)
    ...
    test "expired entries return :miss" do
      ...
      expired_timestamp = System.monotonic_time(:millisecond) - 7_200_000  # 2 hours ago
      :ets.insert(:test_schema_cache, {{schema_module, data_hash}, validation_result, expired_timestamp})
  ...
  describe "cache management" do
    test "can clear entire cache" do
      ...
      assert SchemaRegistry.get_cached_validation(TestSchema1, "hash1", :test_schema_cache) == {:hit, {:ok, %{}}}
      
      # Clear cache (for test table, manually clear)
      :ets.delete_all_objects(:test_schema_cache)
      
      # Verify entries are gone
      assert SchemaRegistry.get_cached_validation(TestSchema1, "hash1", :test_schema_cache) == :miss
      assert SchemaRegistry.get_cached_validation(TestSchema2, "hash2", :test_schema_cache) == :miss
    end
    ...
    test "provides cache statistics" do
      ...
      assert stats.max_size > 0  # Production default is 10,000
  ...
  describe "LRU eviction" do
    test "evicts least recently used entries when max size is reached" do
      ...
      for i <- 1..10 do
        SchemaRegistry.cache_validation_result(:"TestSchema#{i}", "hash#{i}", {:ok, %{id: i}}, :test_schema_cache)
      end
      
      Process.sleep(10)
      
      # Verify all entries are cached
      assert SchemaRegistry.get_cached_validation(:TestSchema1, "hash1", :test_schema_cache) == {:hit, {:ok, %{id: 1}}}
      assert SchemaRegistry.get_cached_validation(:TestSchema10, "hash10", :test_schema_cache) == {:hit, {:ok, %{id: 10}}}
      
      # Add one more entry (should trigger eviction)
      SchemaRegistry.cache_validation_result(:TestSchema11, "hash11", {:ok, %{id: 11}}, :test_schema_cache)
      ...
      assert SchemaRegistry.get_cached_validation(:TestSchema11, "hash11", :test_schema_cache) == {:hit, {:ok, %{id: 11}}}
    end
  end
