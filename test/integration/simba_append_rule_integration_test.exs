    ...
    test "append_rule strategy generates meaningful instruction improvements" do
      ...
      training_data = [
        ...
        %{
          ...
          outputs: %{
            answer: "Rain is caused by water vapor in clouds condensing and falling to Earth."
          }
        }
      ]
      ...
      metric_fn = fn example, prediction ->
        ...
        if String.length(prediction.answer) > 20 &&
             String.contains?(
               ...
               String.downcase(example.outputs.answer)
             ),
           do: 1.0,
           else: 0.0
      end
      ...
      # AppendRule strategy should now work
      result =
        Simba.compile(simba, original_program, original_program, training_data, metric_fn, [])
