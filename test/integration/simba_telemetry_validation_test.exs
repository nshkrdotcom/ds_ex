defmodule DSPEx.Integration.SimbaTelemetryValidationTest do
  ...
  setup do
    ...
    # Attach telemetry handlers for SIMBA events
    telemetry_events = [
      [:dspex, :simba, :optimization, :start],
      [:dspex, :simba, :optimization, :stop],
      [:dspex, :simba, :optimization, :error],
      [:dspex, :simba, :strategy, :start],
      [:dspex, :simba, :strategy, :stop],
      [:dspex, :simba, :strategy, :skip],
      [:dspex, :simba, :trajectory, :sample],
      [:dspex, :simba, :trajectory, :evaluate],
      [:dspex, :simba, :bucket, :analyze],
      [:dspex, :simba, :performance, :measure],
      [:dspex, :simba, :append_rule, :start],
      [:dspex, :simba, :append_rule, :stop],
      [:dspex, :simba, :append_rule, :error],
      [:dspex, :teleprompter, :compile, :start],
      [:dspex, :teleprompter, :compile, :stop]
    ]
  ...
  describe "SIMBA optimization telemetry" do
    test "emits comprehensive optimization lifecycle events" do
      ...
      # Verify optimization lifecycle events
      optimization_start_events = filter_events(events, [:dspex, :simba, :optimization, :start])
      optimization_stop_events = filter_events(events, [:dspex, :simba, :optimization, :stop])
      ...
      assert_telemetry_event_structure(start_event, [:dspex, :simba, :optimization, :start])

      {_event, measurements, metadata} = start_event
      assert Map.has_key?(measurements, :system_time)
      assert Map.has_key?(metadata, :strategy_count)
      assert Map.has_key?(metadata, :training_size)

      # Verify optimization stop event structure
      stop_event = hd(optimization_stop_events)
      assert_telemetry_event_structure(stop_event, [:dspex, :simba, :optimization, :stop])

      {_event, measurements, metadata} = stop_event
      assert Map.has_key?(measurements, :duration)
      assert Map.has_key?(metadata, :success)
      assert Map.has_key?(metadata, :final_score)
    ...
    test "emits strategy-specific telemetry events" do
      ...
      strategy_start_events = filter_events(events, [:dspex, :simba, :strategy, :start])
      strategy_stop_events = filter_events(events, [:dspex, :simba, :strategy, :stop])

      assert length(strategy_start_events) >= 1, "Should emit strategy start events"

      # Verify strategy event metadata
      strategy_events = strategy_start_events ++ strategy_stop_events
      strategy_names = Enum.map(strategy_events, fn {_, _, metadata} ->
        Map.get(metadata, :strategy_name)
      end) |> Enum.filter(& &1 != nil)

      assert :append_demo in strategy_names, "Should emit append_demo strategy events"
      # append_rule events depend on successful trajectory variance detection
    ...
    test "emits AppendRule-specific telemetry during instruction generation" do
      ...
      responses = [
        %{content: "4"},      # Should match
        %{content: "wrong"},  # Should not match
        %{content: "10"}      # Should match
      ]
      DSPEx.MockClientManager.set_mock_responses(:test, responses)
      ...
      append_rule_start_events = filter_events(events, [:dspex, :simba, :append_rule, :start])
      append_rule_stop_events = filter_events(events, [:dspex, :simba, :append_rule, :stop])
      append_rule_error_events = filter_events(events, [:dspex, :simba, :append_rule, :error])

      # Verify AppendRule events (may not always trigger if no trajectory variance)
      total_append_rule_events = length(append_rule_start_events) +
      ...
      if total_append_rule_events > 0 do
        ...
        IO.puts("No AppendRule telemetry events (trajectory variance may not have been sufficient)")
      end
    ...
    test "emits trajectory sampling and evaluation telemetry" do
      ...
      trajectory_sample_events = filter_events(events, [:dspex, :simba, :trajectory, :sample])
      trajectory_evaluate_events = filter_events(events, [:dspex, :simba, :trajectory, :evaluate])

      assert length(trajectory_sample_events) >= 1, "Should emit trajectory sampling events"
      assert length(trajectory_evaluate_events) >= 1, "Should emit trajectory evaluation events"

      # Verify trajectory event structure
      if length(trajectory_sample_events) > 0 do
        {_event, measurements, metadata} = hd(trajectory_sample_events)
        assert Map.has_key?(measurements, :system_time)
        assert Map.has_key?(metadata, :example_id) || Map.has_key?(metadata, :trajectory_id)
      end

      if length(trajectory_evaluate_events) > 0 do
        {_event, measurements, metadata} = hd(trajectory_evaluate_events)
        assert Map.has_key?(measurements, :evaluation_score) || Map.has_key?(measurements, :score)
        assert Map.has_key?(metadata, :success) || Map.has_key?(metadata, :evaluation_result)
      end
    ...
    test "emits performance measurement telemetry" do
      ...
      performance_events = filter_events(events, [:dspex, :simba, :performance, :measure])
      ...
      if length(performance_events) > 0 do
        ...
        assert Map.has_key?(metadata, :measurement_type) || Map.has_key?(metadata, :performance_type),
  ...
  describe "error scenario telemetry" do
    test "emits telemetry for optimization errors" do
      ...
      # Verify error events
      optimization_error_events = filter_events(events, [:dspex, :simba, :optimization, :error])
    ...
    test "emits telemetry for strategy failures and skips" do
      ...
      strategy_skip_events = filter_events(events, [:dspex, :simba, :strategy, :skip])
  ...
  describe "telemetry correlation and metadata" do
    test "telemetry events include proper correlation IDs" do
      ...
      optimization_events = filter_events(events, [:dspex, :simba, :optimization, :start]) ++
                           filter_events(events, [:dspex, :simba, :optimization, :stop])

      if length(optimization_events) >= 2 do
        correlation_ids = Enum.map(optimization_events, fn {_, _, metadata} ->
    ...
    test "telemetry metadata includes comprehensive context" do
      ...
      # Should include SIMBA configuration context
      expected_metadata_fields = [
        :strategy_count, :training_size, :num_candidates, :max_steps,
        :temperature, :client_type, :signature_type
      ]

      present_fields = Enum.filter(expected_metadata_fields, fn field ->
  ...
  defp collect_events(events, 0), do: Enum.reverse(events)
