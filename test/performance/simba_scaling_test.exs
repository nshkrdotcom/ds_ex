defmodule DSPEx.Performance.SimbaScalingTest do
  ...
  describe "SIMBA optimization time scaling" do
    test "optimization time scales sub-quadratically with training set size" do
      ...
      Enum.each(results, fn result ->
        if result.success do
          IO.puts("Dataset #{result.dataset_size}: #{Float.round(result.time_ms, 1)}ms (#{Float.round(result.time_per_example, 2)}ms/example)")
        else
    ...
    test "optimization with both strategies scales appropriately" do
      ...
      simba_single = SIMBA.new(
        strategies: [:append_demo],
        num_candidates: 4,
        max_steps: 2
      )

      simba_both = SIMBA.new(
      ...
      {single_time, single_result} = :timer.tc(fn ->
      ...
      {both_time, both_result} = :timer.tc(fn ->
      ...
      case {single_result, both_result} do
        {{:ok, single_opt}, {:ok, both_opt}} ->
          ...
          IO.puts("Single strategy score: #{Float.round(single_opt.performance.average_score, 3)}")
    ...
    test "concurrent trajectory sampling scales with candidate count" do
      ...
      results = Enum.map(candidate_counts, fn num_candidates ->
        simba = SIMBA.new(
          strategies: [:append_demo],
      ...
      successful_results = Enum.filter(results, fn r ->
        match?({:ok, _}, r.result)
      end)

      assert length(successful_results) >= 2, "Need successful results to analyze candidate scaling"
      ...
      Enum.each(results, fn result ->
        case result.result do
          {:ok, _} ->
            IO.puts("#{result.num_candidates} candidates: #{Float.round(result.time_ms, 1)}ms (#{Float.round(result.time_per_candidate, 2)}ms/candidate)")
          {:error, _} ->
  ...
  describe "memory usage scaling validation" do
    test "memory usage scales linearly with dataset size" do
      ...
      memory_results = Enum.map(dataset_sizes, fn size ->
      ...
      Enum.each(memory_results, fn result ->
        IO.puts("Dataset #{result.dataset_size}: #{Float.round(result.memory_growth_mb, 2)}MB total (#{Float.round(result.memory_per_example, 3)}MB/example)")
      end)
    end
    ...
    test "memory cleanup after optimization cycles" do
      ...
      cycle_memories = Enum.map(1..5, fn cycle ->
        simba = SIMBA.new(
          strategies: [:append_demo],
          num_candidates: 5,
          max_steps: 2
        ...
        responses = Enum.map(1..@medium_dataset_size, fn i -> %{content: "Cycle #{cycle} Answer #{i}"} end)
        DSPEx.MockClientManager.set_mock_responses(:test, responses)
      ...
      avg_earlier = Enum.sum(Enum.map(earlier_cycles, fn {_, mem} -> mem end)) / length(earlier_cycles)
  ...
  describe "concurrent optimization performance" do
    test "multiple concurrent SIMBA optimizations don't interfere" do
      ...
      optimization_tasks = Enum.map(1..optimization_count, fn task_id ->
        Task.async(fn ->
          simba = SIMBA.new(
            strategies: [:append_demo],
            num_candidates: 4,
      ...
      successful_tasks = Enum.filter(concurrent_results, fn {_, result, _} ->
        match?({:ok, _}, result)
      end)
      ...
      if length(successful_tasks) > 0 do
        ...
        IO.puts("Tasks: #{optimization_count}, Successful: #{length(successful_tasks)}, Failed: #{failure_count}")
    ...
    test "SIMBA performance under resource contention" do
      ...
      background_tasks = Enum.map(1..10, fn i ->
        Task.async(fn ->
          # Simulate CPU and memory pressure
          large_list = Enum.map(1..10000, fn j -> "Background task #{i} item #{j}" end)
      ...
      responses = Enum.map(1..@large_dataset_size, fn i -> %{content: "Contention Answer #{i}"} end)
      DSPEx.MockClientManager.set_mock_responses(:test, responses)
      ...
      optimization_duration_ms = System.convert_time_unit(end_time - start_time, :native, :millisecond)
  ...
  defp create_scaling_trainset(size) do
    Enum.map(1..size, fn i ->
      ...
      {question, answer} = case question_type do
        0 -> {"What is #{i} + #{i}?", "#{i * 2}"}
