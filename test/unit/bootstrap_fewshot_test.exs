defmodule DSPEx.Teleprompter.BootstrapFewShotTest do
  ...
  describe "compile/5 with struct" do
    test "successfully optimizes student program", %{teacher: teacher, student: student, trainset: trainset, metric_fn: metric_fn} do
      teleprompter = BootstrapFewShot.new(max_bootstrapped_demos: 2)
      
      {:ok, optimized} = BootstrapFewShot.compile(teleprompter, student, teacher, trainset, metric_fn, [])
  ...
  describe "compile/5 with options" do
    ...
    test "handles progress callback", %{teacher: teacher, student: student, trainset: trainset, metric_fn: metric_fn} do
    ...
    test "returns error for invalid inputs" do
      {:error, :invalid_student_program} = BootstrapFewShot.compile(
        "invalid", 
        %MockTeacherProgram{}, 
        [], 
        fn _, _ -> 1.0 end
      ...
      {:error, :invalid_teacher_program} = BootstrapFewShot.compile(
      ...
      {:error, :invalid_or_empty_trainset} = BootstrapFewShot.compile(
        %MockStudentProgram{demos: []}, 
        %MockTeacherProgram{}, 
        [], 
        fn _, _ -> 1.0 end
      ...
      {:error, :invalid_metric_function} = BootstrapFewShot.compile(
        %MockStudentProgram{demos: []}, 
        %MockTeacherProgram{}, 
        [Example.new(%{})], 
        "invalid"
    ...
    test "handles teacher failures gracefully", %{student: student, trainset: trainset, metric_fn: metric_fn} do
      failing_teacher = %FailingTeacherProgram{}
      
      {:error, :no_successful_bootstrap_candidates} = BootstrapFewShot.compile(
        student, 
        failing_teacher, 
        trainset, 
        metric_fn,
    ...
    test "filters out low quality demonstrations", %{teacher: teacher, student: student, trainset: trainset} do
      ...
      {:ok, optimized} = BootstrapFewShot.compile(
        ...
        trainset, 
        low_quality_metric,
        quality_threshold: 0.8  # Higher than what metric returns
      )
    ...
    test "limits number of demonstrations", %{teacher: teacher, student: student, trainset: trainset, metric_fn: metric_fn} do
      {:ok, optimized} = BootstrapFewShot.compile(
        student, 
        teacher, 
        trainset, 
        metric_fn,
        max_bootstrapped_demos: 1  # Limit to 1 demo
      )
  ...
  describe "program without native demo support" do
    ...
    test "wraps program without demo support", %{teacher: teacher, trainset: trainset, metric_fn: metric_fn} do
  ...
  describe "edge cases" do
    ...
    test "handles concurrent processing limits", %{teacher: teacher, student: student, trainset: trainset, metric_fn: metric_fn} do
      {:ok, optimized} = BootstrapFewShot.compile(
        student, 
        teacher, 
        trainset, 
        metric_fn,
        max_concurrency: 1,  # Very limited concurrency
        max_bootstrapped_demos: 1
      )
