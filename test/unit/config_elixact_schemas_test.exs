defmodule DSPEx.Config.ElixactSchemasTest do
  ...
  describe "schema definitions" do
    ...
    test "schemas can validate correct data structures" do
      ...
      assert {:ok, _} =
               PredictionConfiguration.validate(%{
    ...
    test "schemas reject invalid data structures" do
      ...
      assert {:error, _} =
               PredictionConfiguration.validate(%{default_provider: :invalid_provider})
  ...
  describe "path to schema mapping" do
    test "maps client configuration paths correctly" do
      ...
      assert {:ok, ClientConfiguration, [:backoff_factor]} =
               ElixactSchemas.path_to_schema([:dspex, :client, :backoff_factor])
    end
    ...
    test "maps provider configuration paths with wildcards" do
      ...
      assert {:ok, ProviderConfiguration, [:api_key]} =
               ElixactSchemas.path_to_schema([:dspex, :providers, :openai, :api_key])

      assert {:ok, ProviderConfiguration, [:api_key]} =
               ElixactSchemas.path_to_schema([:dspex, :providers, :claude, :api_key])
  ...
  describe "configuration value validation" do
    test "validates simple configuration values" do
      ...
      assert {:error, _} =
               ElixactSchemas.validate_config_value([:dspex, :telemetry, :enabled], "true")
    end
    ...
    test "validates choice constraints" do
      ...
      assert {:error, _} =
               ElixactSchemas.validate_config_value([:dspex, :logging, :level], :invalid_level)
    ...
    test "validates numeric constraints" do
      ...
      assert {:error, _} =
               ElixactSchemas.validate_config_value([:dspex, :client, :retry_attempts], -1)
    end
  ...
  describe "edge cases and error handling" do
    ...
    test "handles empty and malformed paths" do
      ...
      assert {:error, :unknown_path} =
               ElixactSchemas.path_to_schema([:not_dspex, :client, :timeout])
    end
    ...
    test "validates schema consistency across different providers" do
      ...
      Enum.each(providers, fn provider ->
        ...
        assert :ok =
                 ElixactSchemas.validate_config_value(
                   [:dspex, :providers, provider, :api_key],
                   "test-key"
                 )
    ...
    test "handles Union type validation for API keys" do
      ...
      assert {:error, _} =
               ElixactSchemas.validate_config_value([:dspex, :providers, :gemini, :api_key], 123)
