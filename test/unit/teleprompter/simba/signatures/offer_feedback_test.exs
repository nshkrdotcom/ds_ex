defmodule DSPEx.Teleprompter.SIMBA.Signatures.OfferFeedbackTest do
  ...
  describe "OfferFeedback signature" do
    ...
    test "defines module_advice output field" do
      ...
      module_advice_field = definition.outputs[:module_advice]
      assert module_advice_field.type == :map
    end
    ...
    test "integrates with DSPEx signature system" do
      ...
      assert OfferFeedback.instructions() != nil
      assert String.length(OfferFeedback.instructions()) > 0
    end
    ...
    test "supports signature compilation and validation" do
      ...
      {:ok, validated_inputs} = OfferFeedback.validate_inputs(inputs)
      
      assert validated_inputs.program_code == inputs.program_code
      assert validated_inputs.modules_defn == inputs.modules_defn
      assert String.length(validated_inputs.better_program_trajectory) >= 50
      assert String.length(validated_inputs.worse_program_trajectory) >= 50
    end
