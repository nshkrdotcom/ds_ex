defmodule DSPEx.Teleprompter.SIMBA.Strategy.AppendDemoTest do
  ...
  defp mock_append_demo_apply(bucket_data, program, opts \\ %{}) do
    ...
    cond do
      ...
      true ->
        # Try to convert best trajectory to demo
        best_trajectory = get_best_trajectory(bucket_data)

        case mock_create_demo_from_trajectory(best_trajectory) do
          {:ok, _demo} ->
            enhanced_program =
              Map.merge(program, %{
                enhanced: true,
          ...
          {:error, reason} ->
            {:skip, "demo conversion failed: #{reason}"}
        end
    end
  end
