defmodule DSPEx.Teleprompter.SIMBA.Strategy.AppendRuleTest do
  ...
  describe "AppendRule strategy" do
    ...
    test "apply generates instruction improvements from trajectory analysis" do
      ...
      assert String.length(enhanced_program.instruction) >
               String.length(source_program.instruction)

      # Verify instruction contains improvement guidance
      keywords = ["precise", "double-check", "calculations", "arithmetic"]

      assert Enum.any?(keywords, fn keyword ->
               String.contains?(enhanced_program.instruction, keyword)
             end)
    end
  ...
  describe "AppendRule edge cases and error handling" do
    test "handles extremely large trajectory datasets efficiently" do
      ...
      large_trajectory_set =
        Enum.map(1..1000, fn i ->
          # Create variance pattern
          success = rem(i, 3) == 0
    ...
    test "gracefully degrades with malformed trajectory data" do
      # Test with various malformed trajectory structures - all must be valid Trajectory structs
      # with valid numeric scores (since Bucket.new calculates averages)
      malformed_trajectories = [
        # Nil values in program/inputs
        %Trajectory{
          program: nil,
          example: create_test_example(),
          inputs: nil,
      ...
      # Note: Bucket.new expects valid Trajectory structs with numeric scores
    ...
    test "validates OfferFeedback signature response format strictly" do
      ...
      Enum.each(invalid_response_clients, fn invalid_client ->
        ...
        assert {:skip, reason} = result
        # Check if reason contains any of the expected keywords
        assert Enum.any?(
                 ["parse", "format", "invalid", "response", "malformed", "meaningful"],
                 fn keyword -> String.contains?(reason, keyword) end
               ),
               "Expected reason '#{reason}' to contain one of: parse, format, invalid, response, malformed, meaningful"
      end)
    end
    ...
    test "handles edge cases in trajectory variance detection" do
      ...
      test_cases = [
        ...
        # All high scores but no success/fail variance - should not detect variance
        {[0.99, 1.0, 0.99, 1.0], false},
        ...
        # High scores with explicit success/fail mix for variance detection
        {[0.99, 1.0, 0.99, 1.0], true, [true, false, true, false]}
      ]

      Enum.each(test_cases, fn test_case ->
        {scores, expected_variance, explicit_success} =
          case test_case do
            {scores, expected_variance, explicit_success} ->
              {scores, expected_variance, explicit_success}

            {scores, expected_variance} ->
              {scores, expected_variance, nil}
          end

        trajectories =
          Enum.with_index(scores, fn score, i ->
            success_value =
              if explicit_success do
                Enum.at(explicit_success, i, score > 0.5)
              else
                score > 0.5
              end
    ...
    fn _signature, _inputs, _options ->
      {:ok,
       %{
         module_advice: %{
           "main" =>
             "Be more precise with numerical calculations. Always double-check arithmetic."
         }
       }}
    end
  end
