defmodule DSPEx.EvaluateSuiteTest do
  ...
  setup do
    devset = [
      DSPEx.Example.new(%{question: "What is 1+1?", answer: "2"}),
      DSPEx.Example.new(%{question: "What is 2+2?", answer: "4"}),
      DSPEx.Example.new(%{question: "What is the capital of France?", answer: "Paris"}),
      DSPEx.Example.new(%{question: "What is 3+3?", answer: "6"})  # Will be wrong
    ]
  ...
  describe "evaluation execution" do
    ...
    test "handles program failures gracefully", %{devset: devset} do
      failing_devset = devset ++ [DSPEx.Example.new(%{question: "fail", answer: "anything"})]
    ...
    test "runs evaluation concurrently", %{devset: _devset} do
      # Create a larger dataset to test concurrency
      large_devset = for i <- 1..20 do
        DSPEx.Example.new(%{question: "What is #{i}+#{i}?", answer: "#{i*2}"})
      end
  ...
  describe "metric functions" do
    test "supports different metric types" do
      examples = [
        DSPEx.Example.new(%{question: "Test", answer: "correct"}),
        DSPEx.Example.new(%{question: "Test2", answer: "wrong"})
      ]
    ...
    test "handles metric exceptions" do
      ...
      evaluator = DSPEx.Evaluate.new(
        devset: [DSPEx.Example.new(%{question: "test", answer: "test"})],
